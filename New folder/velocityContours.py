...
def read_csv(number):
    ...
    arr = np.loadtxt('csv_000'+str(Number)+'.csv', dtype=float, delimiter=';', skiprows=4, usecols=(0,1,2,3,4,5,6,8))
    #small files

    ## picked 0_x,1_y,2_z,3_idp,4_vx, 5_vy, 6_vz and 7_type
...
# find the particles within the range
def find_range(arr):
    index_range = np.where((arr[:,1]>=2.001) & arr[:,7]<=2.003)
    reduced_arr = arr[index_range]
    return reduced_arr
