...
def read_csv(first,last):
    ...
    arr2 = np.loadtxt('draftsplash_000'+str(first)+'.csv', dtype=float, delimiter=';', skiprows=4, usecols=(0,2,3,8))
    arr1 = np.loadtxt('draftsplash_00'+str(last)+'.csv', dtype=float, delimiter=';', skiprows=4, usecols=(0,2,3,8))
    #small files
...
def multi_fx(count,arr1,arr2,h,T):
    ...
    #where displacement is not equal to 0
    temp_indices2 = arr1_temp[:,3]!=0
    temp_indices2 = arr2_temp[:,3]!=0
