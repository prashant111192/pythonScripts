...
y_shift_array = np.linspace(1,0.7,5)
...
np.savetxt(f"./figs2/percent.csv",  average_percent_arr[:len(heights_array)-1,:],header = ','.join([str(yy) for yy in y_shift_array]),comments='', delimiter=',')
np.savetxt(f"./figs2/percent_no_outliers.csv",  average_percent_arr[len(heights_array):,:],header = ','.join([str(yy) for yy in y_shift_array]),comments='', delimiter=',')
